...
char * get_answer(char *coordinates){
    FILE* stream;
    stream = fopen("/home/rohit/Desktop/track_data.csv", "r");
    char line[1024];
    int mat[100][5];
    int t=0;
		...
		if ( isCommCapable ) {
			...
			if ( isRunning ) {
				do {
         			int iter = -1;
          		int start = 0;
              int coordinates_done;
              int first_32_bits_done;
              ...
              char *encrypted_states2;
              	...
              	if(coordinates_done==1){
                    ...
                    char q_1[11];
                    sprintf(q_1,"%d",p_1);
  									line = concat(concat("w",concat(q_1," ")),encrypted_states1);
                  printf("%s\n", line);
  									add_history(line);
                  ...
                  printf("%s\n",line);
                    ...
                    char q_1[11];
                    sprintf(q_1,"%d",p_1);
                    line = concat(concat("w",concat(q_1," ")),encrypted_states2);
  									add_history(line);
  								printf("%s\n",line );
  									pStatus = parseLine(handle, line, &error);
  									CHECK_STATUS(pStatus, pStatus, cleanup);
                ...
                printf("%s\n",line);
                  ...
                  char q_1[11];
                  sprintf(q_1,"%d",p_1);
  								line=concat(concat("w",concat(q_1," ")), encrypted_data);
  								add_history(line);
                printf("%s\n",line);
                	pStatus = parseLine(handle, line, &error);
  								CHECK_STATUS(pStatus, pStatus, cleanup);
                  ...
                  printf("%s\n", f2hData);
                	...
                	if(strcmp(decrypted_data,"00100001001000010010000100100001") ==0)  /** ack1 from fpga right or not**/
                  	...
                  	map[iter].chnl_no = 2*iter;
			              char *encrypted_ack2=encrypt("00000000000000000000000000000001");
              			line=concat(concat("w",concat(q_1," ")),encrypted_ack2);
			              add_history(line);
                  printf("%s\n",line);
  									pStatus = parseLine(handle, line, &error);
  									CHECK_STATUS(pStatus, pStatus, cleanup);
